...
const pool = new Pool({
    user: "postgres.oijlmzyykkvxmqxjitna",
    host: "aws-0-ap-south-1.pooler.supabase.com",
    database: "postgres",
    password: "Singham202@1234",
    port: 6543,
...
app.get("/create-table", async (req, res) => {
  ...
  `;
    ...
    try {
        ...
        res.json({
            message: "Users table created in api_schema or already exists.",
        });
    } catch (err) {
        console.error("Error creating table:", err);
        res
            .status(500)
            .json({ message: "Failed to create users table", error: err.message });
    }
});
...
app.post("/users", async (req, res) => {
    ...
    const insertUserQuery =
        "INSERT INTO api_schema.users (name, email, password) VALUES ($1, $2, $3) RETURNING *";
...
app.post("/login", async (req, res) => {
    ...
    try {
        ...
        // Optionally, you can return a token or user data here
        res.status(200).json({
...
app.get("/create-trades-table", async (req, res) => {
  ...
  `;
    ...
    try {
        ...
        res.json({
            message: "Trades table created or already exists.",
        });
    } catch (err) {
        console.error("Error creating trades table:", err);
        res
            .status(500)
            .json({ message: "Failed to create trades table", error: err.message });
    }
});
...
app.get("/portfolio/:userId", async (req, res) => {
    ...
    SELECT stock_symbol, SUM(CASE WHEN trade_type = 'buy' THEN quantity ELSE -quantity END) AS quantity
